...
- **`pncp.ts`**: `parseNumeroControlePNCP` testado com casos válidos/inválidos (função pura, sem mock necessário). `buscarContratacaoPNCP` testado com `fetch` mockado via `vi.stubGlobal("fetch", ...)` — **mesmo padrão já estabelecido em `brasilapi.test.ts`/`viacep.test.ts`** (resposta de sucesso mapeada corretamente, resposta de erro/`!ok` retorna `null`). Isso corrige uma primeira versão desta spec que sugeria testes sem mock contra a API real — o codebase já tem convenção estabelecida de mockar `fetch` nos testes automatizados de clientes externos, reservando a chamada real pra verificação manual.
- **`licitacaoService.ts`**: testado contra o Prisma de teste real, com `buscarContratacaoPNCP`/`buscarContratacaoPNCP` (importado de `pncp.ts`) mockado via `vi.mock`/`vi.spyOn` — cadastro (rejeita formato inválido, rejeita duplicata, rejeita não encontrado no PNCP, cria com sucesso), atualização de dados do PNCP, atualização de dados internos (não mexe no snapshot), listagem/busca, exclusão.
